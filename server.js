...
app.get('/', requiresAuth, (req, res) => {
  ...
  for (const [name, tag] of Object.entries(CRON_TAGS)) {
    paused[name] = cronLines.some(l => l.includes(tag) && l.trim().startsWith('#'));
  ...
  const messages = req.session.messages || [];
  req.session.messages = [];
  res.render('index', { scripts: SCRIPTS, logs: LOGS, cron_output: cronOutput, paused, last_status, messages });
});
...
app.get('/run/:name', requiresAuth, (req, res) => {
  ...
  if (script) {
    try {
      spawn('sudo', ['/bin/bash', script], { detached: true, stdio: 'ignore' }).unref();
...
app.get('/toggle_cron/:name', requiresAuth, (req, res) => {
  ...
  flash(req, 'success', `${resumed ? '▶️ Resumed' : '⏸️ Paused'} cron job for '${name}'.`);
  res.redirect('/');
});

app.post('/update', requiresAuth, (req, res) => {
  try {
    spawn('/bin/bash', ['/home/pi/pi-script-dashboard/update_dashboard.sh'], { detached: true, stdio: 'ignore' }).unref();
